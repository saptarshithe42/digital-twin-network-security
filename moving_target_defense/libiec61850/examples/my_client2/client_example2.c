...
int main(int argc, char **argv)
{
    ...
    if (error == IED_ERROR_OK)
    {
        ...
        IedConnection_writeObject(con, &error, "IED01LogicalDevice/GGIO1.CurrentPressure.mag.f", IEC61850_FC_MX, pressureValue);

        if (error != IED_ERROR_OK)
            printf("failed to write IED01LogicalDevice/GGIO1.CurrentPressure.mag.f! (error code: %i)\n", error);
        else
        {
