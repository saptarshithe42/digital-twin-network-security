...
class MovingTargetDefense(app_manager.RyuApp):
    ...
    def TimerEventGen(self):
        """
        A function which generates timeout events every SHUFFLE_INTERVAL seconds
    ...
    def handlePacketInEvents(self, ev):
        ...
        if udp_pkt and udp_pkt.src_port == 53 and ip.tos == 0:
            ...
            eth_reply = ethernet.ethernet(
                dst=eth.dst, src=eth.src, ethertype=eth.ethertype
            )
            ...
            """
            if self.isRealIPAddress(src) and src not in list(
            ...
            if self.isVirtualIPAddress(dst):
                ...
                if self.isDirectContact(
                    ...
                    print(
                        ("Changing DST Virtual IP " + dst + "---> REAL DST IP " + tpa)
                    )
