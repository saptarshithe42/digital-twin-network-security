...
class MovingTargetDefense(app_manager.RyuApp):
    ...
    def handleSwitchFeatures(self, ev):
        ...
        self.add_dns_request_intercept_flow_rule(datapath)
    ...
    def update_resources(self, ev):
        ...
        for curSwitch in self.datapaths:
            ...
            self.add_dns_request_intercept_flow_rule(curSwitch)
    ...
    def add_dns_request_intercept_flow_rule(self, datapath, priority=200):
        ...
        match_dns = parser.OFPMatch(
            eth_type=0x0800,  # IPv4
            ip_proto=17,  # UDP
            udp_dst=53,  # Destination port 53 (DNS request)
            # ipv4_src="10.0.0.9",
            ip_dscp=0,
        )
    ...
    def handlePacketInEvents(self, ev):
        ...
        # dns request
        if udp_pkt and udp_pkt.dst_port == 53 and ip.tos == 0:
            dns_request = True
            self.logger.info("-----------dns request intercepted--------------")

            ip.dst = self.R2V_Mappings[self.dns_server_real_ip]
            ip.tos = 16
            pkt.serialize()
        ...
        # dns response
        elif udp_pkt and udp_pkt.src_port == 53 and ip.tos == 0:
            dns_response = True
